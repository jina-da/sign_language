...
# 데이터 활용 안내 (Data Notice)

## 사용 데이터셋

본 프로젝트는 **AI Hub의 한국수어 영상 데이터셋**을 학습 및 테스트 데이터로 활용하였습니다.
...
AI Hub 데이터셋은 **연구 및 학습 목적**으로 사용이 허용됩니다.  
저작권 정책에 따라 **원본 데이터셋은 본 레포지토리에 포함되어 있지 않습니다.**
...
2. 회원가입 및 로그인
3. 검색창에 **"한국수어 영상"** 검색
4. 신청서 제출 후 승인 시 다운로드 가능

---

## 데이터 배치 방법

다운로드한 영상 파일을 아래 경로에 위치시켜야 합니다.

```
video-server/
└── static/
    └── videos/
        ├── NIA_SL_WORD0001_SYN01_F.mp4
        ├── NIA_SL_WORD0002_SYN01_F.mp4
        └── ...
```

### 파일명 규칙

- 형식: `NIA_SL_WORD{번호}_SYN{버전}_F.mp4`
- WORD0001 ~ WORD0551 → `SYN01`
- WORD0552 이상 → `SYN02`
...
- 본 데이터셋을 외부에 재배포하는 것은 **엄격히 금지**되어 있습니다.
- 반드시 AI Hub에서 직접 신청 및 다운로드하여 사용하시기 바랍니다.
